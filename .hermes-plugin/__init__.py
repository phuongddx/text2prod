...
BOOTSTRAP_MARKER = "text2prod:using-text2prod bootstrap for hermes"


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install text2prod`): the plugin
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, "using-text2prod", "SKILL.md")):
            return cand
    raise RuntimeError(
        "text2prod plugin: cannot find the skills/ tree "
        f"(looked at {candidates}). Reinstall with "
        "`hermes plugins install text2prod`."
    )
...
def _build_bootstrap(skills_dir: str) -> str:
    with open(
        os.path.join(skills_dir, "using-text2prod", "SKILL.md"),
        encoding="utf-8",
    ) as f:
        body = _strip_frontmatter(f.read())

    tools_path = os.path.join(
        skills_dir, "using-text2prod", "references", "hermes-tools.md"
    )
    ...
    return (
        ...
        f"{BOOTSTRAP_MARKER}\n\n"
        f"You have text2prod.\n\n"
        f"The using-text2prod skill content is included below and is already "
        f"loaded for this Hermes session. Follow it now. "
        f"Do not try to load using-text2prod again.\n\n"
        f"{body}\n\n"
        f"## Loading Text2Prod Skills on Hermes\n\n"
        f"Text2Prod skills are registered with Hermes' native skill loader: "
        f'invoke one with `skill_view("text2prod:skill-name")` '
        f'(for example `skill_view("text2prod:brainstorming")`). '
        f"If a namespaced lookup returns 'not found', read the skill file "
        f"directly instead:\n"
        f'`read_file("{skills_dir}/skill-name/SKILL.md")`\n\n'
        f"The text2prod skills directory is: `{skills_dir}`\n\n"
        f"{tool_mapping}\n"
        f"</EXTREMELY_IMPORTANT>"
    )
