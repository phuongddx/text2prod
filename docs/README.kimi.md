# Text2Prod for Kimi Code

Complete guide for using Text2Prod with [Kimi Code](https://github.com/MoonshotAI/kimi-code).
...
2. Loads `using-text2prod` at session start through `sessionStart.skill`.
3. Provides Kimi-specific tool mapping through `skillInstructions`.

Kimi Code reads Text2Prod skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Text2Prod and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info text2prod` and check diagnostics.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next Text2Prod release, install the branch explicitly:
...
1. Confirm `/plugins info text2prod` shows the plugin enabled.
