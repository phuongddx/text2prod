...
Text2Prod is opinionated about worktree management — specific paths (`.worktrees/<branch>`), specific commands (`git worktree add`), specific cleanup (`git worktree remove`). Meanwhile, Claude Code, Codex App, Gemini CLI, and Cursor all provide native worktree support with their own paths, lifecycle management, and cleanup.
...
For harnesses without native support (Codex CLI, OpenCode, Copilot standalone), text2prod fills a real gap. The skill shouldn't go away — it should get out of the way when native support exists.
...
- `.text2prod-session.json` metadata tracking (interesting PR #997 idea, not needed for v1)
...
Whoever creates the worktree owns its cleanup. If the harness created it, text2prod doesn't touch it. If text2prod created it (via git fallback), text2prod cleans it up. The heuristic: if the worktree lives under `.worktrees/` or `worktrees/`, text2prod owns it. Anything else (`.claude/worktrees/`, `~/.codex/worktrees/`, `.gemini/worktrees/`, or old user-global Text2Prod paths) belongs to the harness or user and is left alone.
...
No interactive directory selection prompt. Old user-global Text2Prod worktree paths are not detected or offered; new manual worktrees are project-local unless the user explicitly specifies another location.
...
git worktree remove "$WORKTREE_PATH"  # only if text2prod owns it
...
**Option 4 (Discard):** Require typed "discard" confirmation. Then remove worktree (if text2prod owns it), force-delete branch.
...
if worktree path is under .worktrees/ or worktrees/:
    # Text2Prod created it — we own cleanup
...
The `.worktrees/` or `worktrees/` = ours, anything else = hands off` heuristic works for every current harness. If a future harness adopts one of those project-local directories as its convention, we'd have a false positive (text2prod tries to clean up a harness-owned worktree). Similarly, if a user manually runs `git worktree add .worktrees/experiment` without text2prod, we'd incorrectly claim ownership. Both are low risk — every harness uses branded paths, and manual `.worktrees/` creation is unlikely — but worth noting.
