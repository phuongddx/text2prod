...
**Goal:** Make text2prod defer to native harness worktree systems when available, fall back to manual git worktrees when not, and fix three known finishing bugs.
...
**Spec:** `docs/text2prod/specs/2026-04-06-worktree-rototill-design.md`
...
**If worktree path is under `.worktrees/` or `worktrees/`:** Text2Prod created this worktree — we own cleanup.
...
```markdown
- **text2prod:using-git-worktrees** - REQUIRED: Set up isolated workspace before starting
```

to:

```markdown
- **text2prod:using-git-worktrees** - Ensures isolated workspace (creates one or verifies existing)
```
