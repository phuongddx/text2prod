# Porting Text2Prod to a New Harness

This guide explains how to add support for a new harness — an IDE, CLI, or
agent runner that isn't Claude Code — so that Text2Prod skills auto-trigger
...
## Part 1 — How Text2Prod works across harnesses

Text2Prod is the same content everywhere. What changes per harness is the thin
...
2. **Tool mapping (per-harness).** Each harness needs the action vocabulary
   ...
   `skills/using-text2prod/references/<harness>-tools.md` and/or inline in the
...
3. **Bootstrap (per-harness).** At the start of every session, the full
   `skills/using-text2prod/SKILL.md` is injected into the model's context,
...
A harness can support Text2Prod only if it can do all of the following. Check
...
If the only way to get Text2Prod in front of the model is for your human
...
1. The `using-text2prod` bootstrap loads at session start, every session, with
...
model to describe its text2prod. If the bootstrap injected, it knows it has
them. (OpenCode's install doc uses `opencode run --print-logs "hello" 2>&1 |
grep -i text2prod` for the same goal via a different mechanism — log-grep
...
`using-text2prod` skill's own description at session start). If more than one
...
`using-text2prod/SKILL.md` and prints a JSON object whose **field name and
...
- Reference: `.opencode/plugins/text2prod.js` (JavaScript) and
  `.pi/extensions/text2prod.ts` (TypeScript). pi is the closest reference for
...
- Reference: `gemini-extension.json` (manifest, with `contextFileName`),
  ...
  reference), `skills/using-text2prod/references/gemini-tools.md`.
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  - **Build/dependency check.** Decide how the harness loads your module:
    ...
    step? Text2Prod is zero-runtime-dependency. pi's `import type
...
`using-text2prod` skill content (wrapped in `<EXTREMELY_IMPORTANT>` tags) plus
...
text2prod… for all other skills use the Skill tool" preamble, escapes it, and
...
field/nesting, matcher strings — are the harness's contract, not Text2Prod',
...
pi's just says "do not try to load using-text2prod again." If your harness has
...
*not* the user's own global file) pulls in two things: the `using-text2prod`
skill and the harness's tool-mapping reference. `GEMINI.md`
does this with two `@`-includes (`@./skills/using-text2prod/SKILL.md` and
`@./skills/using-text2prod/references/<harness>-tools.md`); the harness loads
...
- **Shape A:** put it in `skills/using-text2prod/references/<harness>-tools.md`.
...
- **Shape B:** the mapping is typically inlined into the bootstrap string you
  inject (see the `toolMapping` constant in `text2prod.js`). pi keeps it in
...
`using-text2prod/SKILL.md` tells the model to *never read skill files manually
...
2. **Native skill *discovery* but no `Skill` tool** (pi, Antigravity): the harness
   ...
   **For the bootstrap itself, prefer a declared context file (Part 6).** If the
   ...
   carries both the `using-text2prod` content and the tool mapping. That is the
   ...
   **Fallback — the surfaced skill index.** If there's no context-file field but
   ...
   is the index, and `using-text2prod`'s own surfaced description can be what
   ...
   - **It bootstraps *triggering*, not the *tool mapping*.** An injector prepends
     `<harness>-tools.md` alongside `using-text2prod` every session. Here nothing
...
3. **No skill system at all:** there is nothing to register, and the *only*
   ...
   what it can't find: `using-text2prod/SKILL.md` does **not** enumerate the
...
# 3. Smoke check: does the model know it has text2prod?
...
tmux send-keys -t port-test 'What are your text2prod?'; sleep 0.4; tmux send-keys -t port-test Enter
sleep 5
...
If the smoke check shows the model *doesn't* know it has text2prod, the
...
- **A plugin installer may silently strip *undeclared* files — so make the
  ...
  - **Ship a context file the manifest declares.** If the harness has a
    ...
    live `using-text2prod/SKILL.md` + the tool mapping (wrapped in
    ...
    `✔ context : ANTIGRAVITY.md`, and a clean session reads `using-text2prod`'s
    ...
    unrecognized.
  - **Otherwise lean on the installed `using-text2prod` skill itself.** If the
    harness surfaces each installed skill's name + description at session start,
    the `using-text2prod` description ("Use when starting any conversation…")
...
- Fill in the PR template's **"New harness support"** section and paste the
  ...
  be closed.
- Text2Prod is a zero-dependency plugin. Don't add a third-party runtime
...
- **Opt-in isn't a port.** If your human partner has to do anything per session
  to get Text2Prod, the acceptance test fails. Re-read Part 2.
