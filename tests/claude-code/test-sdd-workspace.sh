...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.text2prod/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.text2prod/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.text2prod/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.text2prod/sdd/.gitignore" && "$(cat "$repo/.text2prod/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .text2prod/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .text2prod/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.text2prod/sdd/plan-a/task-1-brief.md" ]]; then
    ...
    case "$rp_path" in
        "$repo/.text2prod/sdd/plan-a/review-"*.diff)
    ...
    if [[ "$wt_dir" == "$wt_root/.text2prod/sdd/plan-a" && "$wt_dir" != "$dir_a" ]]; then
