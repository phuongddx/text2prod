...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$TEXT2PROD_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$TEXT2PROD_SKILLS_DIR/using-text2prod/SKILL.md" "$TEST_HOME/using-text2prod.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$TEXT2PROD_PLUGIN_FILE" missing
}
