#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the text2prod plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/text2prod.js"
...
skill_count=$(find "$TEXT2PROD_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $TEXT2PROD_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-text2prod skill exists (critical for bootstrap)
echo "Test 3: Checking using-text2prod skill (required for bootstrap)..."
if [ -f "$TEXT2PROD_SKILLS_DIR/using-text2prod/SKILL.md" ]; then
    echo "  [PASS] using-text2prod skill exists"
else
    echo "  [FAIL] using-text2prod skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$TEXT2PROD_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/text2prod/' "$TEXT2PROD_PLUGIN_FILE"; then
