...
# 1. Create in text2prod location (lowest priority)
mkdir -p "$TEXT2PROD_SKILLS_DIR/priority-test"
cat > "$TEXT2PROD_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Text2Prod version of priority test skill
---
# Priority Test Skill (Text2Prod Version)

This is the TEXT2PROD version of the priority test skill.

PRIORITY_MARKER_TEXT2PROD_VERSION
EOF
...
if [ -f "$TEXT2PROD_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Text2Prod version exists"
else
    echo "  [FAIL] Text2Prod version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled text2prod priority
echo ""
echo "Test 2: Documenting personal vs text2prod priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled text2prod version instead of personal version"

# Test 3: Document project vs bundled text2prod priority
echo ""
echo "Test 3: Documenting project vs personal/text2prod priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled text2prod version instead of project version"

# Test 4: Test a non-colliding bundled text2prod skill is still available
echo ""
echo "Test 4: Testing non-colliding text2prod skill remains available..."

mkdir -p "$TEXT2PROD_SKILLS_DIR/text2prod-only-test"
cat > "$TEXT2PROD_SKILLS_DIR/text2prod-only-test/SKILL.md" <<'EOF'
---
name: text2prod-only-test
description: Text2Prod-only priority test skill
---
# Text2Prod Only Test Skill

PRIORITY_MARKER_TEXT2PROD_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"text2prod-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_TEXT2PROD_ONLY_VERSION" "Non-colliding text2prod skill is still registered"
