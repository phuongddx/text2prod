...
class TestSkillsDirResolution:
    def test_repo_layout_resolves(self):
        ...
        assert os.path.isfile(
            os.path.join(skills, "using-text2prod", "SKILL.md")
        )
...
class TestBootstrapContent:
    ...
    def test_contains_using_text2prod_body(self):
        content = _bootstrap()
        # A distinctive line from the skill body proves the real SKILL.md was
        # embedded, not a stub.
        assert "You have text2prod" in content
    ...
    def test_tool_mapping_sourced_from_reference_file(self):
        ...
        content = _bootstrap()
        ref = os.path.join(
            m._skills_dir(), "using-text2prod", "references", "hermes-tools.md"
        )
    ...
    def test_skill_view_guidance_present(self):
        content = _bootstrap()
        assert 'skill_view("text2prod:brainstorming")' in content
