...
const extensionPath = resolve(repoRoot, '.pi/extensions/text2prod.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-text2prod/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'text2prod');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/text2prod.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have text2prod/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have text2prod/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have text2prod/);
