#!/usr/bin/env bash
# Validate the Devin CLI integration. `devin plugins install ./text2prod`
...
if manifest.get("name") != "text2prod":
    raise AssertionError(f"plugin name: expected 'text2prod', got {manifest.get('name')!r}")
