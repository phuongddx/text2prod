...
To invoke a text2prod skill, use:
...
If `skill_view` cannot find a text2prod skill (it may not appear in the catalog
until the plugin fully registers it), fall back to reading the SKILL.md directly:

```
read_file(path="~/.hermes/plugins/text2prod/skills/<skill-name>/SKILL.md")
```
