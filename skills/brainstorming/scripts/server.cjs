...
const TEXT2PROD_VERSION = readText2ProdVersion();
const TELEMETRY_DISABLE_ENV_VARS = [
  'TEXT2PROD_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const TEXT2PROD_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(TEXT2PROD_VERSION);
  return '<div class="brand"><span class="brand-copy">Text2Prod v' + version + '</span></div>';
}
