/**
 * Text2Prod plugin for OpenCode.ai
 *
 * Injects text2prod bootstrap context via message transform.
...
export const Text2ProdPlugin = async ({ client, directory }) => {
  const homeDir = os.homedir();
  const text2prodSkillsDir = path.resolve(__dirname, '../../skills');
  ...
  const getBootstrapContent = () => {
    ...
    // Try to load using-text2prod skill
    const skillPath = path.join(text2prodSkillsDir, 'using-text2prod', 'SKILL.md');
...
${toolMapping}
  ...
  return {
    // Inject skills path into live config so OpenCode discovers text2prod skills
    ...
    config: async (config) => {
      ...
      if (!config.skills.paths.includes(text2prodSkillsDir)) {
        config.skills.paths.push(text2prodSkillsDir);
      }
    },
